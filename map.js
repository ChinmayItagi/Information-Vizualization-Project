...
var queue = d3.queue();
queue.defer(d3.json, "Data/crime-in-los-angeles/lapd-divisions.geojson")
queue.defer(d3.json, "Data/crime-in-los-angeles/dataByAreaV2.json")
queue.defer(d3.json, "Data/crime-in-los-angeles/dataOverall.json")
queue.defer(d3.json, "Data/crime-in-los-angeles/timeOccurredTotal.json")
queue.defer(d3.json, "Data/crime-in-los-angeles/timeOccurredTotalByYear.json")
queue.defer(d3.json, "Data/crime-in-los-angeles/timeOccurredByArea.json")
queue.await(ready);
...
dispatch.on("load.map", function () {
    ...
    dispatch.on("statechange.map", function (y) {
        ...
        paths.transition().duration(800).attr("fill", function (d) {
           console.log(d.properties.byYear[y][0]['count']); return color(d.properties.byYear[y][0]['count']);
        })
...
dispatch.on("load.pie", function () {
    ...
    var color = d3.scaleOrdinal()
        .domain(groups)
        .range(["#0000EE", "#1B3F8B", "#0276FD", "#4E78A0", "#63D1F4", "#C1F0F6", "#388E8E"]);
